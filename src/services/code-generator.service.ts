...
export class CodeGeneratorService {
     ...
     * @param prefix - Tiền tố (PD, ST, IM, EX, UN, US)
     * @returns Mã code mới (VD: PD0001)
     */
    async generateNextCode(entityName: string, codeField: string, prefix: string): Promise<string> {
        try {
     ...
     * @returns Mã sản phẩm mới (VD: PD0001)
     */
    async generateProductCode(): Promise<string> {
        return this.generateNextCode('TB_PRODUCT', 'PD_CD', 'PD');
    }
